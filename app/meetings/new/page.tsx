...
const MEETING_ID_KEY = "meetingai:active-meeting-id";

function getMeetingId(): string {
  const existing = window.sessionStorage.getItem(MEETING_ID_KEY);
  if (existing) return existing;
  const id = createId("meeting");
  window.sessionStorage.setItem(MEETING_ID_KEY, id);
  return id;
}

function defaultTitle(): string {
  return `새 회의 ${new Date().toLocaleString("ko-KR", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
...
export default function NewMeetingPage() {
  const meetingId = useSyncExternalStore(
    () => () => {},
    getMeetingId,
    () => "meeting_pending",
  );
